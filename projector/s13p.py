...
def _cube_face_planes(corners, tol=1e-14):
    """
    corners: (8,3)
    Returns outward face planes n·x + c = 0,
    with inside cube satisfying n·x + c <= tol.
    """
    ...
    for i, face in enumerate(_CUBE_FACES):
        p0, p1, p2, _ = corners[face]
        n = np.cross(p1 - p0, p2 - p0)
        nn = np.linalg.norm(n)
        if nn < tol:
            raise ValueError("Degenerate cube face.")
        n = n / nn

        cc = -np.dot(n, p0)

        # orient outward so center is inside: n·center + c <= 0
        if np.dot(n, center) + cc > 0.0:
            n = -n
            cc = -cc
...
def _origin_in_cube(face_normals, face_c, tol=1e-10):
...
def _cube_fully_outside_cone(corners, side_normals, tol=1e-10):
    """
    If there exists one cone side plane such that all cube corners
    are strictly outside that plane, then cube and cone do not intersect.
    """
    vals = corners @ side_normals.T   # (8,4)
...
def _classify_pixels_for_zone(zone_corners, cand_side_normals, tol=1e-10):
    ...
    Returns:
        ...
        need_exact:  (Np,)
    """
    # vals[p, i_corner, j_plane] = zone_corners[i_corner] · cand_side_normals[p, j_plane]
    vals = np.einsum('ic,pjc->pij', zone_corners, cand_side_normals)  # (Np,8,4)

    # all corners inside all cone side planes
    full_inside = np.all(vals >= -tol, axis=(1, 2))

    # exists a plane for which all corners are outside
...
def cube_cone_intersection_vertices_precomputed(
    ...
    if _origin_in_cube(face_normals, face_c, tol=tol):
...
def project(cube, xyz, dxyz, proj_center, proj_axis,
    ...
    Implemented speedups:
      1) cheap full reject
      2) per-zone cube geometry precomputed once
      3) batched pixel classification (inside/outside/exact)
      4) cached HEALPix geometry by NSIDE
    """
    ...
    for izone in range(Nz):
        if 1:
            print("s11p Izone/Nzone %d/%d = %0.2f" % (izone, Nz, izone / max(Nz, 1)))
        ...
        face_normals, face_c = _cube_face_planes(zone_corners)
        ...
        for ipix in pix_exact:
            ...
            try:
                # extra safety; with batched classification this should usually be unnecessary,
                # but keep it for robustness near tolerance boundaries.
