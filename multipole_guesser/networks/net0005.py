...
idd = 5
what = "More sphereical iterations, try to speed up"
...
class LocalSphereAttention(nn.Module):
    def __init__(self, dim, n_heads, k):
        ...
        # Cache dicts keyed by device and N (number of points)
        self.register_buffer('cached_idx', None, persistent=False)
        self.register_buffer('cached_bias', None, persistent=False)
        self.cache_initialized = False

    def gather_neighbor_xyz(self, xyz, idx):
        B, N, _ = xyz.shape
        k = idx.size(-1)
        batch_indices = torch.arange(B, device=xyz.device).view(B, 1, 1).expand(B, N, k)
        neighbor_xyz = xyz[batch_indices, idx, :]  # [B, N, k, 3]
        return neighbor_xyz

    def compute_and_cache(self, xyz):
        """
        Compute neighbor indices and positional biases once for fixed xyz.
        """
        with torch.no_grad():
            ...
            rel_pos_flat = rel_pos.view(-1, 3)  # [(B*N*k), 3]
            bias_out = self.bias_mlp(rel_pos_flat)  # [(B*N*k), H]
            bias_out = bias_out.view(B, N, k, self.n_heads).permute(0, 3, 1, 2)  # [B, H, N, k]

        # Store cache as buffers (no gradients)
        self.cached_idx = idx
        self.cached_bias = bias_out
    ...
    def forward(self, x, xyz):
        ...
        if (not self.cache_initialized) or (self.cached_idx is None) or (self.cached_idx.shape[1] != N) or (self.cached_idx.device != xyz.device):
            self.compute_and_cache(xyz)

        idx = self.cached_idx  # [B, N, k]
        bias_out = self.cached_bias  # [B, H, N, k]
        ...
        # Gather neighbor k and v features with efficient indexing
        batch_indices = torch.arange(B, device=x.device).view(B, 1, 1).expand(B, N, k)
        head_indices = torch.arange(self.n_heads, device=x.device).view(1, 1, 1, self.n_heads).expand(B, N, k, self.n_heads)
        # We will do gather manually across N dimension:

        # For each head, gather neighbor features for k and v
        # k_feat: [B, N, H, D] --> permute to [B, H, N, D]
...
class main_net(nn.Module):
    def __init__(self, input_dim=3, embed_dim=64, depth=4, n_heads=8, k=15, num_outputs=15):
        ...
        self.blocks = nn.ModuleList([SphereformerBlock(embed_dim, n_heads, k) for _ in range(depth)])
    ...
    def forward(self, sky):
        ...
        # Convert spherical coords to cartesian (fixed per datum)
        xyz = torch.stack([
