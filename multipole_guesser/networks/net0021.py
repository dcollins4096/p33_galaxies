...
import pdb
idd = 21
what = "2d conv network. Horse around"

def thisnet(Nell):

    model = main_net(Nell, hidden_channels=512, num_encodings=16)
...
def trainer(
    ...
    *,
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer,
        milestones=[75], #[100,300,600],  # change after N and N+M steps
...
class main_net(nn.Module):
    def __init__(self, Nell, hidden_channels=64, num_encodings=4, layers=[256,512,256]):
        super().__init__()
        ...
        self.head = nn.Sequential(
            ...
            nn.Flatten(),              # [B,hidden]
            nn.Linear(hidden_channels, 2 * num_coeffs)  # [B,2*n_coeff]
        )
    ...
    def forward(self, x):
        ...
        out = self.head(feats)  # [B,2*n_coeff]
        out = out.view(-1, 2, self.n_coeff)
        out = out[:,0,:]+1j*out[:,1,:] #make it complex for symmetry going forward.
